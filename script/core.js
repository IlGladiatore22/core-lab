...
function initUser(requireAdmin, callback) {
    var u = null;
    try { var d = localStorage.getItem('corelab_user'); u = d ? JSON.parse(d) : null; } catch(e) { u = null; }

    if (!u) {
        var uid = getCookie('corelab_uid');
        if (uid) { loadUserFromBin(uid, requireAdmin, callback); return true; }
    ...
    if (requireAdmin) {
        checkAdmin(u.id).then(function(isAdmin) {
            if (!isAdmin) { window.location.href = 'home.html'; return; }
            showUser(u);
...
async function loadUserFromBin(uid, requireAdmin, callback) {
    try {
        var res = await fetch(BIN_URL, { headers: { 'X-Master-Key': API_KEY } });
        if (!res.ok) return;
        var data = (await res.json()).record;
        ...
        if (found) {
            ...
            showUser(found);
            if (requireAdmin) {
                checkAdmin(found.id).then(function(isAdmin) {
                    if (!isAdmin) { window.location.href = 'home.html'; return; }
                    if (callback) callback();
                });
            } else {
...
function showUser(u) {
    ...
    var nu = document.getElementById('navUser'), mu = document.getElementById('mobileUser');
    var lo = document.getElementById('logoutBtn'), ml = document.getElementById('mobileLogout');
...
function timeAgo(ts) {
    var diff = Date.now() - ts; var s = Math.floor(diff / 1000);
    if (s < 60) return 'ora'; var m = Math.floor(s / 60);
    if (m < 60) return m + 'm fa'; var h = Math.floor(m / 60);
    if (h < 24) return h + 'h fa'; var d = Math.floor(h / 24);
    if (d < 30) return d + 'g fa'; return Math.floor(d / 30) + ' mesi fa';
}

function showToast(msg, type) {
    var toast = document.getElementById('toast');
    if (!toast) { toast = document.createElement('div'); toast.id = 'toast'; toast.className = 'toast'; document.body.appendChild(toast); }
    toast.textContent = msg; toast.className = 'toast ' + (type || 'info') + ' show';
    setTimeout(function() { toast.classList.remove('show'); }, 2500);
}

async function saveBin() {
    var res = await fetch('https://api.jsonbin.io/v3/b/' + BIN_ID, {
        method: 'PUT', headers: { 'X-Master-Key': API_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify(mainData)
    });
